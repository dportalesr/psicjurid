...
echo
	$this->element('adminhdr',array(
		'links'=>array(
			array('text'=>'Regresar','href'=>array('controller'=>'projects','action'=>'index','admin'=>1),'class'=>'back'),
			'add'
		),
		'title'=>'Descargables — '.$project_name
	)),
	$this->element('pages');
		...
		$html->tag('table',null,array('class'=>'datagrid')),
			...
			foreach($items as $it){
				...
				echo $html->tableCells(array(array(
					...
					array(
						$html->link('Descargar',array('action'=>'download','admin'=>0,$id),array('target'=>'_blank')).
