...
<?=$html->css(array('generic','main','pulsembox','http://fonts.googleapis.com/css?family=Signika:600,300'))?> 
</head>
<?php
echo
	$html->tag('body',null,'c_'.$this->params['controller'].' a_'.$this->params['action']),
		$html->div('outside exposure',''),
		$html->div('outside',''),
		$html->div(null,null,array('id'=>'nofooter')),
			$html->div(null,null,array('id'=>'header')),
				$html->div('clear'),
					$html->div('social'),
						$html->link('facebook','http://facebook.com',array('target'=>'_blank','rel'=>'nofollow')),
						$html->link('twitter','http://twitter.com',array('target'=>'_blank','rel'=>'nofollow')),
					'</div>',
					$html->tag('h2',$html->link($sitename_for_layout,'/',array('title'=>$sitename_for_layout)),array('id'=>'logo')),
					$html->div('header-info'),
						$html->tag('h1','Instituto Interdisciplinario de Psicología Jurídica'),
						$html->para(null,'Al servicio de la razón para el bien común'),
					'</div>',
				'</div>',
			'</div>',
			$this->element('menu'),
